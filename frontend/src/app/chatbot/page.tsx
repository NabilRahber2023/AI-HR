...
export default function UserChatbotPage() {
  const WELCOME: Message = {
    role: 'assistant',
    content: "Hello! I'm your personal HR AI Assistant 👋 I can help you with performance improvement, career growth, skill development, and more. What would you like to explore today?",
    time: fmtTime(),
    aiPowered: false,
  }
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-screen overflow-hidden bg-surface-container-low">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 custom-scrollbar">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-4 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse ml-auto' : ''}`}>
              <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-sm ${msg.role === 'assistant' ? 'bg-primary text-on-primary' : 'bg-primary-container text-on-primary-container'}`}>
                {msg.role === 'assistant' ? '🤖' : '👤'}
              </div>
              <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`p-4 rounded-2xl shadow-sm font-body-md text-body-md leading-relaxed ${msg.role === 'user' ? 'bg-primary text-on-primary rounded-tr-none' : 'bg-surface-container-lowest border border-outline-variant/30 text-on-surface rounded-tl-none'}`}>
                  {msg.content}
                </div>
